...
void set_codec_params(AVFormatContext *&fctx, AVCodecContext *&codec_ctx, double width, double height, int fps, int bitrate)
{
  ...
  codec_ctx->flags |= AV_CODEC_FLAG_GLOBAL_HEADER | AV_CODEC_FLAG_LOW_DELAY;

  // codec_ctx->me_range

  fctx->max_interleave_delta = 1000;
}
...
{
  SwsContext *swsctx = sws_getContext(width, height, AV_PIX_FMT_NV12,
...
AVFrame *allocate_frame_buffer(AVPixelFormat pix_fmt, double width, double height)
{
  ...
  int i = av_image_get_buffer_size(pix_fmt, width, height, 1);
  uint8_t *framebuf = new uint8_t[i];

  av_image_fill_arrays(frame->data, frame->linesize, framebuf, pix_fmt, width, height, 1);
  frame->width = width;
  frame->height = height;
